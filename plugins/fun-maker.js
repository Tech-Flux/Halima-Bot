...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    switch (command) {
      ...
      conn.sendFile(m.chat, img, 'img.png', `✅ Done...`, m)
      m.react(done)
      break 
      case 'rip':
      let ri = global.API('fgmods', '/api/maker/rip', { url: pp }, 'apikey')
      conn.sendFile(m.chat, ri, 'img.png', `✅ Done...`, m)
      m.react(done)
      break
      case 'shit':
      let sh = global.API('fgmods', '/api/maker/shit', { url: pp }, 'apikey')
      conn.sendFile(m.chat, sh, 'img.png', `✅ Done...`, m)
      m.react(done)
      break 
      case 'spank':
      case 'azotar':
      if (!who2) return m.reply(`✳️ Use as Follows : ${usedPrefix + command} @tag`)
      let az = global.API('fgmods', '/api/maker/spank', { url: me, url2: pp }, 'apikey')
      conn.sendFile(m.chat, az, 'img.png', `✅ Done...`, m)
      m.react(done)
      break  
      case 'imgdel':
      let del = global.API('fgmods', '/api/maker/delete', { url: pp }, 'apikey')
      conn.sendFile(m.chat, del, 'img.png', `✅ Done...`, m)
      m.react(done)
      break 
      case 'beautiful':
      let bea = global.API('fgmods', '/api/maker/beautiful', { url: pp }, 'apikey')
      conn.sendFile(m.chat, bea, 'img.png', `✅ Done...`, m)
      m.react(done)
      break 
