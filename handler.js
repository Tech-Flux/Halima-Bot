    ...
    try {
        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (xp > 200)
                    m.reply('Squek -_-') // Hehehe
                else
                    m.exp += xp
                if (!isPrems && plugin.diamond && global.db.data.users[m.sender].diamond < plugin.diamond * 1) {
                    this.reply(m.chat, `✳️ Your diamonds have run out. Use the following command to purchase more diamonds\n\n*${usedPrefix}buy*`, m)
                    continue // Limit habis
                }
                if (plugin.level > _user.level) {
                    this.reply(m.chat, `✳️ nivel requerido ${plugin.level} To use this command. \nreach Level ${_user.level}`, m)
...
if (groupUpdate.icon) text = (chats.sIcon || this.sIcon || conn.sIcon || 'The group icon changed').replace('@icon', groupUpdate.icon)
if (groupUpdate.revoke) text = (chats.sRevoke || this.sRevoke || conn.sRevoke || 'The group link changed to\n@revoke').replace('@revoke', groupUpdate.revoke)
        if (!text) continue
...
global.dfail = (type, m, conn) => {
    let msg = {
        rowner: `👑 Only Abdul can do this.`,
        owner: `👑 Only Abdul can do this.`,
        mods: `🔰`,
        premium: `💠 Purchase premium first`,
        group: `⚙️ Can only be used in groups`,
